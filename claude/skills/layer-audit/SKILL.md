...
## Related: Import Cycle Detection

Import cycles (A imports B imports A) are a separate concern from layer violations but often co-occur. The desloppify `cycles` detector catches these automatically during `desloppify scan`. For manual checking:

```bash
# Check for circular imports (requires madge)
pnpx madge --circular src/
```
